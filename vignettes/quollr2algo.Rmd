---
title: "2. Algorithm for visualising the model overlaid with high-dimensional data"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{2. Algorithm for visualising the model overlaid with high-dimensional data}
...
The algorithm begins with 2D embedding data and results a tour which shows the model overlaid with high-dimensional data.

To construct the model in the 2D space, first you need to decide hexagonal grid configurations, which means that the number of bins along the x and y axes. You can start with by calculating the defaults and make the changes to have a representative model later. 

Let's compute with default settings for `hex_size`, `buffer_x`, and `buffer_y`.
...
You need to obtain the low density hexagons.
...
As you see, even though there are low density hexagons, it's not a good choice of removing them. 
...
To visualise what you get, use `geom_trimesh()` just passing the hexagonal bin centroid coordinates and it shows the triangular mesh.
...
To construct the wireframe in 2D, you need to find which vertices are connected. By passing triangular object to `gen_edges`, you can obtain which edges are exists and which vertices are connected.
...
Another important parameter of this algorithm is the benchmark value of removing long edges. To compute it, you first need to generate the 2D Euclidean distance data set for the edges.
...
Then, to compute a default value for the benchmark value, you can pass the `distance_df` to `find_lg_benchmark()`. Again, to obtain a representative model, you can change this value accordingly. This function is only provide a default value.
...
To visualse what you get, you can use `vis_lg_mesh()` and `vis_rmlg_mesh()`. In here, used the benchmark value as $0.75$.
...
Then, you need to pass the generated data set to `avg_highd_data()` to obtain the high-dimensional coordinates of the model.
