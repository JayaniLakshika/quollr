...
#' @param apply_pca Logical indicating whether to apply Principal Component Analysis (PCA) before calculating the averages. Default is TRUE.
#'
...
#'   7, 8, 9, 1
#' )
#' avg_highD_data(df, apply_pca = FALSE)
#'
#' @export
avg_highD_data <- function(.data, apply_pca = TRUE) {
  # Calculate the average values within each hexagonal bin
  df_b <- .data |>
  ...
  # Select specific columns based on apply_pca
  if (isTRUE(apply_pca)) {
    # Column names start with "pc"
    df_b <- df_b |>
...
find_benchmark_value <- function(.data, distance_col) {
  # Round the distances for simplicity
  .data <- .data |>
    dplyr::mutate(dplyr::across({{ distance_col }}, \(x) round(x, 4)))

  # Sort the distances
  sorted_distance_df <- .data |>
    dplyr::arrange({{ distance_col }})

  # Get the unique distances and calculate the differences
  unique_dist <- sorted_distance_df |>
    dplyr::pull({{ distance_col }}) |>
    unique()
  dist_u <- tibble::tibble(unique_dist = unique_dist)
  dist_u <- dplyr::bind_cols(
    dist_u,
    rbind(NA, apply(dist_u, 2, diff)),
    .name_repair = "unique_quiet"
  )

  # Rename the difference column
  names(dist_u)[2] <- "difference"

  # Replace missing values with zero
  dist_u[is.na(dist_u)] <- 0

  benchmark_value_vec <- c()

  # Find the first largest difference
  ...
  benchmark_value_df <- dist_u[which(dist_u$difference == benchmark_value_vec[!is.na(benchmark_value_vec)]), 1]
