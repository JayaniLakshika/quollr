...
#'
#' @param x An object to compute evaluation metrics for.
...
#'
#' This function computes evaluation metrics (Error and HBE) by comparing
#' the high-dimensional data to the predictions obtained from a hex_model object.
#'
#' @param x An object of class \code{hex_model} generated by \code{fit_highd_model()}.
#' @param highd_data A data frame or tibble containing the original high-dimensional data with an \code{ID} column.
...
#'
#' @export
glance.highd_vis_model <- function(x, highd_data, ...) {

  model_2d    <- x$model_2d
  model_highd <- x$model_highd
...
#'
#' This is a generic function to augment datasets with predictions and error metrics
#' from different models, including NLDR models.
#'
#' @param x An object to augment.
#' @param ... Additional arguments passed to methods (e.g., high-dimensional data).
#'
#' @export
augment <- function(x, ...) {
  UseMethod("augment")
...
#'
#' @param x An object of class \code{highd_vis_model} containing the model outputs.
#' @param highd_data A data frame or tibble containing the original high-dimensional coordinates with an \code{ID} column.
#' @param ... Additional arguments (currently unused).
#'
...
#'
#' @export
augment.highd_vis_model <- function(x, highd_data, ...) {
...
#'
#' @param plots A list of plots which include HBE plot and 2-D layouts.
...
#'
#' @export
plot_hbe_layouts <- function(plots, design){
