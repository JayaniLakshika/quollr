...
test_that("generate_eval_df() works", {
  ...
  testthat::expect_snapshot(generate_eval_df(test_data = s_curve_noise_training,
                                             prediction_df = pred_df_test,
                                             df_bin = df_bin, col_start = "x"))


  pred_emb_list_n <- predict_2d_embeddings(test_data = s_curve_noise_test,
                                         df_bin_centroids = df_bin_centroids,
                                         df_bin = df_bin, type_NLDR = "UMAP")
  pred_df_test_n <- as.data.frame(do.call(cbind, pred_emb_list_n))

  testthat::expect_snapshot(generate_eval_df(test_data = s_curve_noise_test,
