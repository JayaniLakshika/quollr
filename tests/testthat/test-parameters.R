test_that("find_benchmark_value() works", {
  ...
  testthat::expect_equal(find_benchmark_value(distance_edges = distance_df,
                                              distance_col = "distance"), 0.6)

  distance_df_n <- distance_df |> dplyr::mutate(distance = dplyr::if_else(dplyr::row_number() == 10, NA_integer_, distance))
  testthat::expect_error(find_benchmark_value(distance_df_n, "distance"))
